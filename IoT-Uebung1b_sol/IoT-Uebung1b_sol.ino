...
byte checkButton() {
  if (!(digitalRead(buttonA))) { // button pressed
    if (!btn_pressed) {
      delay(10);
      btn_pressed = true;
...
void loop() {
  ...
  while (true) {
    ...
    if (checkButton() == BUTTON_EVENT_PRES)
      break;
  ...
  // Start playing
  randomSeed(millis());
  byte diceNumber = random(1, 6);
  Serial.print("diceNumber: ");
  Serial.println(diceNumber);
  byte diceWay = random(10, 50);
  Serial.print("diceWay: ");
  Serial.println(diceWay);

  for (byte i = 0; i < diceWay; i++) {
    runningDice();
    delay (i*10);
  }
